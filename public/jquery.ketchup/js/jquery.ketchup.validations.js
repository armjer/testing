...
$.ketchup.validation('birthday', 'Not selected!', function(form, el, value) {
    //console.log(value);
    //console.log(el);
...
.validation('float', 'Must be digits.', function(form, el, value) {
    return /^[\d\.]+$/.test(value);
})
...
.validation('maxselect', 'Select not more than {arg1} checkboxes.', function(form, el, value, max) {
  ...
  this.bindBrothers(form, el);
    ...
    .validation('match_password', 'Passwords do not match.', function(form, el, value, word) {
        if(el.val() == $(word).val()) {
            return true;
        } else {
            return false;
